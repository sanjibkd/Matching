...
public class RuleSaver {
	...
	public static void saveRule(Rule rule, String allRulesFilePath, Project project) throws IOException {
		...
		for(int i = 1; i < size; i++){
			...
			if(ruleName.equals(rule.getName())) {
				...
				Rule r = new Rule(ruleName, projectName, terms);
				rules.add(r);
			}
		}
	...
	public static void deleteRule(String projectName, Rule rule, Project project) throws IOException {
		String allRulesFilePath = getAllRulesFilePath(projectName);
		deleteRule(rule, allRulesFilePath, project);
	}

	public static void deleteRule(Rule rule, String allRulesFilePath, Project project) throws IOException {
		// delete the rule in place if it exists
		FileReader reader = new FileReader(allRulesFilePath);
		CSVParser parser = new CSVParser(reader);
		List<CSVRecord> records = parser.getRecords();
		reader.close();
