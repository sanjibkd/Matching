...
public class MatcherSaver {
	...
	public static void saveMatcher(Matcher matcher, String allMatchersFilePath, Project project) throws IOException {
		...
		for(int i = 1; i < size; i++){
			...
			if(matcherName.equals(matcher.getName())) {
				...
				Matcher m = new Matcher(matcherName, projectName, rules);
				matchers.add(m);
			}
		}
	...
	public static void deleteMatcher(String projectName, Matcher matcher, Project project) throws IOException {
		String allMatchersFilePath = getAllMatchersFilePath(projectName);
		deleteMatcher(matcher, allMatchersFilePath, project);
	}

	public static void deleteMatcher(Matcher matcher, String allMatchersFilePath, Project project) throws IOException {
		// delete the matcher in place if it exists
		FileReader reader = new FileReader(allMatchersFilePath);
		CSVParser parser = new CSVParser(reader);
		List<CSVRecord> records = parser.getRecords();
		reader.close();
