...
public class CSVUtils {
	...
	public static List<Feature> loadFeatures(Project project, String csvFeatureFilePath) throws IOException {
		...
		try {
			...
			for(int i = 1; i < size; i++){
				CSVRecord rec = records.get(i);
				// feature_name, function_name, attribute1_name,
				// attribute1_type, attribute2_name, attribute2_type
				String featureName = rec.get(0).trim();
				String functionName  = rec.get(1).trim();
				String attribute1Name = rec.get(2).trim();
				String attribute1Type = rec.get(3).trim();
				String attribute2Name = rec.get(4).trim();
				String attribute2Type  = rec.get(5).trim();
				...
				Feature feature = new Feature(featureName, function, projectName,
						attribute1, attribute2);
	...
	public static List<Rule> readRules(Project project, String table1Name,
		...
		try {
			...
			for(int i = 1; i < size; i++){
				...
				Rule rule = new Rule(ruleName, project.getName(), terms); 
				rules.add(rule);
			}
		}
		catch(FileNotFoundException fnfe) {
	...
	public static List<Matcher> loadMatchers(Project project, String csvMatcherFilePath) throws IOException {
		...
		try {
			...
			for(int i = 1; i < size; i++){
				CSVRecord rec = records.get(i);
				// matcher_name, matcher_string
				String matcherName = rec.get(0).trim();
				String matcherString  = rec.get(1).trim();

				String projectName = project.getName();
				List<Rule> rules = ParsingUtils.parseMatcherFromShortDisplayString(project, matcherString);
				Matcher matcher = new Matcher(matcherName, projectName, rules); 
