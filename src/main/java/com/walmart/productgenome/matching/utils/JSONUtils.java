...
public class JSONUtils {
	...
	public static Attribute getAttributeFromJSON(JsonObject attribObj){
	...
	public static Tuple getTupleFromJSON(JsonObject tupleObj, List<Attribute> attributes) throws IOException {
		...
		for (Attribute attribute : attributes) {
			...
			case TEXT:
				if (!tupleObj.isNull(attributeName)) {		
					attributeValue = tupleObj.getString(attributeName);
				}
				data.put(attribute, attributeValue);
				break;
			case INTEGER:
				if (!tupleObj.isNull(attributeName)) {
					attributeValue = tupleObj.getInt(attributeName);
				}
				data.put(attribute, attributeValue);
				break;
			case FLOAT:
				try{
					attributeValue = tupleObj.getJsonNumber(attributeName).doubleValue();
				}
				catch(Exception e) {
					attributeValue = null;
				}
				...
				break;
			case LONG:
				attributeValue = tupleObj.getJsonNumber(attributeName).longValue();
				data.put(attribute, attributeValue);
				break;
			case BOOLEAN:
				attributeValue = tupleObj.getBoolean(attributeName);
				data.put(attribute, attributeValue);
				break;
			default:
				throw new IOException("Attribute type mismatch for attribute: " + attributeName);	
			}
			}
			catch (Exception e) {
				throw new IOException (e.getMessage() + " for attribute: " + attributeName + " and attribute value: " + attributeValue);
			}
		}
	...
	/**
	 * Parses the JSON table file.
	 * @param tableJSON
	 * @return A Table object.
	 * @throws IllegalArgumentException
	 */
	public static Table getTableFromJSON(String tableJSON) throws IllegalArgumentException {
		try {
		JsonReader reader = Json.createReader(new StringReader(tableJSON));
		JsonObject obj = reader.readObject();
		JsonObject tableObj = obj.getJsonObject(TABLE);
		
		if (!tableObj.containsKey(NAME)) {
			throw new IllegalArgumentException("Name attribute is not present in"
					+ "the table info.");
		}
		String name = tableObj.getString(NAME);
		
		if (!tableObj.containsKey(DESCRIPTION)) {
			throw new IllegalArgumentException("Description attribute is not"
					+ "present in the table info.");
		}
		String description = tableObj.getString(DESCRIPTION);
		
		if (!tableObj.containsKey(DESCRIPTION)) {
			throw new IllegalArgumentException("Id attribute is not present in "
					+ "the table info.");
		}
		...
		if (!tableObj.containsKey(PROJECT_NAME)) {
			throw new IllegalArgumentException("Project Name attribute is not"
					+ "present in the table info.");
		}
		String projectName = tableObj.getString(PROJECT_NAME);

		if (!tableObj.containsKey(ATTRIBUTES) || tableObj.isNull(ATTRIBUTES)) {
			throw new IllegalArgumentException("Attributes are not present in the"
					+ " table info.");
		}
		...
		return table;
			...
			throw new IllegalArgumentException("Format of the input file is "
					+ "invalid.\nJsonParsingException: " + jpe.getMessage());
		}
	}
