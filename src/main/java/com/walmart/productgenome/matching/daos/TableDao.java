...
public class TableDao {
	...
	public static void deleteTable(Table table) throws IOException {
		...
		File tableFile = new File(Constants.ROOT_DIR + projectName + "/" +
				tableName + ".table");
		success = tableFile.delete();

		if (!success) {
			throw new IOException("Failed to delete Table " + tableName);
		}

		// update project
		Project project = ProjectDao.open(projectName);
		project.removeUnsavedTable(tableName); // if the table is unsaved
